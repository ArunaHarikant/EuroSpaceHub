...
(function (global) {
  ...
  function coAuthorPicker(selectedIds, viewerId) {
    ...
    return '<div style="display:grid;gap:6px;max-height:190px;overflow:auto;padding:10px;' +
      'border:1px solid var(--line-strong);border-radius:var(--radius-sm);background:var(--surface-3)">' +
  ...
  function suppRow(s, i) {
    return '<div class="field-row" data-supprow style="margin-bottom:10px">' +
      '<div class="field" style="margin:0"><label class="sr-only" for="suppLabel' + i + '">Label</label>' +
        '<input type="text" id="suppLabel' + i + '" name="suppLabel" placeholder="Label (e.g. Supporting dataset)" value="' + esc(s.label || '') + '"></div>' +
      '<div class="field" style="margin:0"><label class="sr-only" for="suppUrl' + i + '">URL</label>' +
      ...
      '<form id="repForm" novalidate>' +
        '<fieldset><legend>Record</legend>' +
          ...
          '<div class="field"><label for="sAbs">Abstract <span class="req">*</span></label>' +
            ...
            '<p class="field__hint">Target approximately ' + ABSTRACT_TARGET + ' words; hard limit ' + ABSTRACT_MAX + '. ' +
              '<span id="absCount" style="font-variant-numeric:tabular-nums"></span></p></div>' +
        ...
        '<fieldset><legend>Authorship</legend>' +
          '<div class="field"><span class="field__label">Lead author</span>' +
            '<p style="margin:0">' + esc(owner ? owner.fullName : '—') +
